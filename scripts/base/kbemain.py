...
from KBEDebug import INFO_MSG, WARNING_MSG, DEBUG_MSG
...
def onBaseAppReady(isBootstrap):
    ...
    if isBootstrap:
        ...
        KBEngine.createEntityAnywhere('RoomMgr')
...
def onBaseAppShutDown(state):
    ...
    if state == 0:
        if hasattr(KBEngine.baseAppData['Lobby'], 'killServer'):
            Utils.baseLobby().killServer()
    # elif state == 1:
    #     g_LobbyPoller.stop()
